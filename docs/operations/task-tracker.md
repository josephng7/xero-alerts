...
|---|---|---|
| xero-oauth-multitenant | pending | OpenSpec **`refactor-xero-oauth-credentials-multitenant`**: credential table + `organizations.credential_id`; callback saves all `/connections`; refresh CAS on credential row. After live demo. |
