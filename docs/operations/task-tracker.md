...
| xero-oauth | Implement OAuth + encrypted token storage + lock-based refresh | done | OAuth connect/callback plus optimistic refresh lock semantics implemented (`add-xero-oauth-foundation`, `add-xero-refresh-lock`) with encrypted persistence and token-version compare-and-swap helper in `lib/xero/refresh.ts`. **Post-demo multitenant refactor complete (`refactor-xero-oauth-credentials-multitenant`):** one credential grant → many tenants; `xero_oauth_credentials` table + `organizations.credential_id`; callback persists all `/connections` via `fetchAllConnections` + `saveXeroOAuthGrant` (transaction); refresh CAS on credential row. Migration `0006_xero_oauth_credentials.sql` includes backfill. |
...
| notify | Build Teams/Email notification job with dedup/digest | done | Completed fanout + durable dedupe integration: `notify_dispatches` persistence (`0003_notify_dispatches.sql`), shared notify worker, and process-event notify handoff (`complete-notify-dedupe-integration`). **Post-demo:** field-level contact/bank detail in channels + alert JSON — OpenSpec `add-notification-field-level-diff` (**pending**). |
...
| ui-datetime-tz | Dashboard/layout datetimes display explicit +08:00 | done | Shared `formatOpsDateTime` + `OPS_DISPLAY_TIME_ZONE` in `lib/ui/format-ops-datetime.tsx`; home dashboard (`app/page.tsx`) + alert detail (`app/alerts/[id]/page.tsx`); short caption on dashboard. Tests: `tests/format-ops-datetime.test.ts`. |
| security | Complete signature checks, RBAC, masking, key-rotation hardening | in_progress | **Wave 3 complete:** split cron/admin secrets + optional previous keys + rotation runbook (`split-internal-api-secrets`). **2026-05-03:** UUID validation before internal `fetch` URLs (`parseAlertId`) to clear CodeQL `js/request-forgery` (alert #4). **2026-05-08:** `pnpm audit` clean — Next 16.2.6 + **temporary** `pnpm.overrides` for transitive **postcss** / **esbuild**; revisit quarterly per **`docs/operations/deps-overrides-revisit.md`**. Broader RBAC (roles beyond caller class) still pending. |
...
|---|---|---|
| xero-oauth-multitenant | done | OpenSpec **`refactor-xero-oauth-credentials-multitenant`**: `xero_oauth_credentials` table; `organizations.credential_id`; callback saves all `/connections` via `fetchAllConnections` + `saveXeroOAuthGrant`; refresh CAS on credential row. Migration `0006_xero_oauth_credentials.sql`. |
| notify-field-level-diff | pending | OpenSpec **`add-notification-field-level-diff`**: structured per-contact/field diff in notify + `alerts.diff` JSON; masking + truncation; dedupe canonicalization. After live demo. |
